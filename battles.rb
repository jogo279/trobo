...
opponents["jaspervdj"] = "./opponents/jaspervdj/MyTronbot"
opponents["ebrahim"] = "./opponents/ebrahim/MyTronBot"
opponents["ExperimentGarden"] = "java opponents/ExperimentGarden/MyTronBot"
opponents["luv2run"] = "lua opponents/luv2run/sandcrawler.lua"
...
opponents.each do |opp_name, opp_exe|
  trobo.each do |trobo_name, trobo_exe|
    Dir.foreach('battle_maps') do |map|
      ...
      out_file = File.open("battles_final/#{trobo_name}_vs_#{opp_name}_on_#{map}", "w")
...
out_file = File.open("battles_final/summary.txt", "w")
